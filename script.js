...
function redeemCode() {
    ...
    db.ref('giftcodes/' + code).once('value', s => {
        ...
        if (s.exists() && c.limit > 0 && (!c.users || !c.users[user.id])) {
            ...
            save();
            tg.showAlert("Chúc mừng! Bạn đã nhận " + c.amount + " vàng");
            document.getElementById('giftcode-input').value = "";
        } else {
            tg.showAlert("Mã không hợp lệ, đã hết lượt hoặc bạn đã nhập rồi!");
        }
    });
}
...
function withdraw() {
    ...
    if (gold > userData.balance || gold <= 0 || !info) return tg.showAlert("Thông tin sai!");
...
function createGiftcode() {
    ...
    tg.showAlert("Đã tạo mã " + code + " thành công!");
}
        ...
        s.forEach(item => {
            ...
            if (d.status === "Pending") {
                list.innerHTML += `<div class="bg-black/40 p-3 rounded-xl text-[10px] border border-white/5">
                    UID: ${d.uid} | Vàng: ${d.gold}<br>${d.info}
...
function approve(key, isDone, uid, gold) {
    if (isDone) {
        ...
        tg.showAlert("Đã hủy và hoàn vàng!");
    }
}
...
function copyLink() {
    ...
    tg.showAlert("Đã copy link mời bạn!");
}
